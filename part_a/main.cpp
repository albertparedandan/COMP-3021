...
int main() {
  ...
  // read training data
  auto training_data = read_dataset("/project/cl/httpd/htdocs/COMP3211_2019Q1_a3/res/traindata_length_4.xml");

  // call the get feature function to transform the data into desired features
  auto training_examples = array_map<vector<string>, vector<feature_t>>(training_data, part_a::get_features);

  // define oracles. oracles are trivially 0s
  vector<feature_t> training_oracles(training_examples.size(), feature_t(0));

  // get the RNN
  part_a::init(training_data);
  ...
  rnn.train(training_examples, training_oracles, part_a::NUM_EPOCHS);

  // read test data
  auto dev_data = read_dataset("/project/cl/httpd/htdocs/COMP3211_2019Q1_a3/res/devdata_length_4.xml");

  // preprocess the test data
  auto dev_examples = array_map<vector<string>, vector<feature_t>>(dev_data, part_a::get_features);
  ...
  cout << "average distance per token: " << total_distance/(dev_data.size()*4) << endl;
}
