...
namespace part_a {
  ...
  transducer_t make_rnn_recognizer_4_inputs() {
    ...
    auto input_pick = make_pick({0, 1, 2, 0, 1, 2, 3});
    ...
    auto timestep_0_to_2 = compose(group(timestep_0_to_1, embedding_layer), group(make_identities(2), compose(cell, copy)));
    auto timestep_0_to_3 = compose(group(timestep_0_to_2, embedding_layer), group(make_identities(3), cell));


    auto output_pick = make_pick({0, 4, 1, 5, 2, 6, 3, 7});

    auto readout_loss = compose(
            group(compose(dense_feedfwd_softmax, make_tensor_neg()),
                  one_hot), make_dot_product());
    
    return compose(input_pick, group(timestep_0_to_3, make_identities(4)), output_pick,
                    group(readout_loss, readout_loss, readout_loss, readout_loss), make_tensor_add(4));
  }
  ...
  vector<feature_t> get_features(const vector<string> &sentence) {
    ...
    // because the RNN expects 4 tokens
    if(sentence.size() == 4) {
      vector<string> result;
      for (int i = 0; i < 4; ++i) {
          char test = (char) sentence[i][0];
          if (isalnum(test)) {
              result.emplace_back(sentence[i]);
          }
          else {
            result.emplace_back("<s>");
          }
      }
      return vector<feature_t>{result[0], result[1], result[2], result[3]};
      //return vector<feature_t>{sentence[0], sentence[1], sentence[2], sentence[3]};
    }
    else {
      throw std::runtime_error("sentences should be in length 4");
    }
  }
